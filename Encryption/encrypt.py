...
#Yo dawg, i heard you like your dictionary, so i got chu a list to put in your dictionary while you dictionary things
jsonList = []

#enable this when we are ready to encrypt EVERYTHING
for file in glob.glob(".\*"):
  #go through the entire folder and ignore the 3 main files
  if file not in [".\\decrypt.py", ".\\encrpyt.py", ".\\topSecretStuff.json"]:
    #break the file name to two parts, the name and extension
    fName, fExt = os.path.splitext(file[2:])
    ...
    #dictionary to put into the list that will go into the JSON, unfortunately we can't put byte into json
    ...
    jsonList.append(topSecretStuff)
